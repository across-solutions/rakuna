...
/**
 * 商品PDFZIPファイルアップロードクラス
 */
class Upload_Pdf_Item {
	 ...
	 * コンストラクタ
	 *
	 * @param array $file 商品PDFZIPファイルパス
	 */
	...
	public function parse() {
		$unzip = new Unzip();
		$pdfs = $unzip->extract($this->file);
		if (empty($pdfs)) {
			$this->set_error('PDFファイルが見つかりません');
			return;
		}

		foreach ($pdfs as $pdf) {
			if ($this->validate_pdf($pdf)) {
				$this->pdfs[] = $pdf;
			}
		}
	...
	public function save() {
		foreach ($this->pdfs as $pdf) {
			$file_info = File::file_info($pdf);
			if (!PDF_Item::create($file_info['filename'], $pdf)) {
				return false;
			}
		}
		return true;
	...
	private function validate_pdf($file) {
		$file_info = File::file_info($file);
		$setting = Config::get('upload.setting.item_pdf');
		...
		if ( ! empty($setting['ext_whitelist']) && ! in_array(strtolower($file_info['extension']), $setting['ext_whitelist'])) {
			$this->set_error('pdfファイルのみ有効です[' . $file_info['basename'] . ']');
			return false;
		}
