...
use Fuel\Core\Input;
/**
 * メール設定-発注受付メール設定コントローラクラス
 */
class Controller_Setting_Mail_Order extends Controller_Setting_Mail_Base {
	
	/**
	 * ページタイトル
	 */
	protected $title = 'メール設定-発注受付用メール設定';
	...
	public function action_save() {
		...
		if (!$this->validate_edit($data)) {
			$this->render($data, 'setting/mail/order/index');
			return;
		}
		...
		Response::redirect('/manage/setting/mail/order');
	}
}