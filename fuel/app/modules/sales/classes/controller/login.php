...
class Controller_Login extends Controller_Base {
	...
	public function action_index() {
		...
		$auto_login_key = Cookie::get(COOKIE_KEY_SALES_AUTO_LOGIN);
		if (!empty($auto_login_key)) {
			$member = $this->get_member_from_key($auto_login_key);
			if (empty($member)) {
				Cookie::delete(COOKIE_KEY_SALES_AUTO_LOGIN);
			} else {
				$auto_login_key = $this->update_auto_login($member);
				if (!empty($auto_login_key)) {;
					Cookie::set(COOKIE_KEY_SALES_AUTO_LOGIN, $auto_login_key, COOKIE_EXPIRATION_AUTO_LOGIN);
				}
				Auth::force_login($member->id);

				Response::redirect('/sales/member');
			}
		}
	 ...
	 */
	private function get_member_from_key($auto_login_key) {
	 ...
	 * 自動ログインキーを更新する
	 *
	 * @param Model_Sales_Representative $member 営業担当アカウント
	 */
